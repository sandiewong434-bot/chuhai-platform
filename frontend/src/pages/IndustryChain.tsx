...
export default function IndustryChain() {
  ...
  // ── 下游 C011 API 数据（整车出口总量及全球排名）──
  const [exportTotalRankData, setExportTotalRankData] = useState<{year: string; exportVolume: number; globalRank: number; globalTotal: number}[]>([])
  const [exportTotalRankLoading, setExportTotalRankLoading] = useState(false)

  // ── 下游 C018 API 数据（全球销量 TOP15 国家及中国品牌市占率）──
  const [globalSalesShareData, setGlobalSalesShareData] = useState<{rank: number; country: string; sales: number; chinaBrands: string[]}[]>([])
  const [globalSalesShareLoading, setGlobalSalesShareLoading] = useState(false)

  // ── 下游 C019 API 数据（千人保有量 vs 渗透率散点图）──
  const [penetrationData, setPenetrationData] = useState<{country: string; ownership: number; penetration: number; region: string}[]>([])
  ...
  useEffect(() => {
    ...
    indicatorApi.getPoints('vehicle_export_top10_brands', { limit: 20 })
      ...
      .catch(err => {
        console.error('C010 API error:', err)
      })
      .finally(() => setExportBrandLoading(false))
  }, [activeTab])

  // C011 整车出口总量及全球排名
  useEffect(() => {
    if (activeTab !== 'downstream') return
    setExportTotalRankLoading(true)
    indicatorApi.getPoints('vehicle_export_global_rank', { limit: 20 })
      .then(res => {
        const items = res.data.items || []
        const arr = items
          .map((item: any) => ({
            year: item.period_date?.slice(0, 4) ?? '',
            exportVolume: item.value ?? 0,
            globalRank: item.dimension_json?.global_rank ?? 0,
            globalTotal: item.dimension_json?.global_total ?? 0,
          }))
          .filter((v: any) => v.year && v.exportVolume > 0)
          .sort((a: any, b: any) => a.year.localeCompare(b.year))
        setExportTotalRankData(arr)
      })
      .catch(err => {
        console.error('C011 API error:', err)
      })
      .finally(() => setExportTotalRankLoading(false))
  }, [activeTab])

  // C018 全球销量 TOP15 国家及中国品牌市占率
  useEffect(() => {
    if (activeTab !== 'downstream') return
    setGlobalSalesShareLoading(true)
    indicatorApi.getPoints('global_sales_china_share', { limit: 50 })
      .then(res => {
        const items = res.data.items || []
        // 按销量降序取 TOP15
        const ranked = items
          .map((item: any) => ({
            country: item.dimension_json?.country || '未知',
            sales: item.value ?? 0,
            chinaBrands: item.dimension_json?.brands || [],
          }))
          .sort((a: any, b: any) => b.sales - a.sales)
          .slice(0, 15)
          .map((item: any, idx: number) => ({ ...item, rank: idx + 1 }))
        setGlobalSalesShareData(ranked)
      })
      .catch(err => {
        console.error('C018 API error:', err)
      })
      .finally(() => setGlobalSalesShareLoading(false))
  }, [activeTab])
  ...
  useEffect(() => {
    ...
    indicatorApi.getPoints('penetration_vs_ownership', { limit: 50 })
      .then(res => {
        const items = res.data.items || []
        const arr = items
          .map((item: any) => ({
            country: item.dimension_json?.country || '未知',
            ownership: item.dimension_json?.ownership_per_1000 ?? 0,
            penetration: item.dimension_json?.penetration_rate ?? 0,
            region: item.dimension_json?.region || '其他',
          }))
          .filter((v: any) => v.ownership > 0 && v.penetration > 0)
          .sort((a: any, b: any) => b.penetration - a.penetration)
        setPenetrationData(arr)
      })
      .catch(err => {
        console.error('C019 API error:', err)
      })
        ...
        <div className="space-y-6">
          ...
          <div className="ch-card-cut">
            <div className="ch-card-cut-inner p-6">
              <div className="flex items-center gap-2 mb-4">
                <div className="ch-title-bar" />
                <h3 className="text-lg font-semibold text-white">整车出口品牌 TOP10（API实时）</h3>
                {exportBrandLoading && <span className="text-xs text-[var(--cyan)]">加载中...</span>}
              </div>
              {exportBrandData.length > 0 ? (
                <div className="space-y-3">
                  {exportBrandData.map((item) => (
                    <div key={item.rank} className="flex items-center gap-3">
                      <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${
                        item.rank <= 3 ? 'bg-[var(--cyan)] text-[#06111e]' : 'bg-white/10 text-[var(--muted-text)]'
                      }`}>
                      ...
                      <div className="flex-1">
                        ...
                        <div className="h-2 bg-white/5 rounded-full overflow-hidden">
                          <div
                            className="h-full bg-gradient-to-r from-[var(--cyan)] to-[var(--teal)] rounded-full"
                            style={{ width: `${Math.min((item.volume / (exportBrandData[0]?.volume || 1)) * 100, 100)}%` }}
                          />
                        </div>
                        <p className="text-xs text-[var(--muted-text)] mt-0.5">{item.note}</p>
                      </div>
